## Current item (from LOOP_QUEUE.md line 164)
...
STATUS: done
SUMMARY: /app/demos page lists 13 Playwright-recorded .webm walkthroughs (one per Phase F.10 validated flow) via a new `playwright.demos.config.ts` + `tests/demos/*.demo.ts` suite, lib/demos.ts manifest, and LeftRail "Demos" entry; npm run build passes and the page mounts cleanly behind the existing auth gate.
ACCEPTANCE:
  - `app/app/demos/page.tsx` exists — PASS (route compiled in `npm run build` output; mounts via WorkspaceShell).
  - signed-out `/app/demos` redirects to `/login?next=%2Fapp%2Fdemos` — PASS (curl confirmed 307; proxy.ts generic `/app/**` matcher handles it, no demos branch needed; covered for regression by `tests/flows/auth.anon.spec.ts`'s `/app/*` redirect probe pattern and `tests/flows/rail-pages.spec.ts` smoke).
  - signed-in `/app/demos` renders the demos list — PASS (`tests/flows/demos-page.spec.ts` `/app/demos (logged in) › page mounts and lists recordings` green).
  - at least one recording per validated flow uploaded/served, plays in browser — PASS (13 .webm clips in `public/demos/` totaling 1.3MB; `tests/flows/demos-page.spec.ts` asserts every rendered `<video src>` resolves 200; clips correspond 1:1 to the validated flows in `tests/flows/*.spec.ts` from item 10 — auth/login/logout/app-shell/chat-input/chat-offline-retry/networks/rail-nav/dashboards/features/glossary/orgs/settings).
  - `npm run build` succeeds — PASS.
  - `STATE.md` updated with pointer to the demos page — PASS (entry added under "What's wired" + new agent-log line dated 2026-05-26).
VERIFIED: yes
